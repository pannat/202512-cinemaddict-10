...
import {Key, render, RenderPosition, SortType} from "../utils";
...
class PageController {
  ...
  render() {
    if (this._data.length) {
      this._sortComponent.setClickHandler(this._onSortClick.bind(this));
      ...
      // if (this._data.length > ShowingMovies.COUNT_BY_BUTTON) {
      //
      //   const onClickButtonShowMore = () => {
      //     const prevMoviesCount = this._showingMoviesCount;
      //     this._showingMoviesCount += ShowingMovies.COUNT_BY_BUTTON;
      //     this._data.slice(prevMoviesCount, this._showingMoviesCount).forEach((movie) => this._renderMovieCard(this._containerSectionAllMovies, movie));
      //
  ...
  _renderCardsForSectionAllMovies(data) {
    data.slice(0, ShowingMovies.COUNT_ON_START).forEach((movie) => this._renderMovieCard(this._containerSectionAllMovies, movie));
    ...
    if (data.length > ShowingMovies.COUNT_BY_BUTTON) {
      ...
      const onClickButtonShowMore = () => {
        const prevMoviesCount = this._showingMoviesCount;
        this._showingMoviesCount += ShowingMovies.COUNT_BY_BUTTON;
        data.slice(prevMoviesCount, this._showingMoviesCount).forEach((movie) => this._renderMovieCard(this._containerSectionAllMovies, movie));
